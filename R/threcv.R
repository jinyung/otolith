...
#'   (e.g. species) based on the calculated posterior probability. New sample is
#'   assigned to the class that has the higher posterior probability. We can
#'   reject the classification result if the posterior probability is low (the
#'   "unsure" classification). In this way we can increase the overall accuracy
#'   (correct predictions out of all the reported results), but the tradeoff is
#'   the decrease in number of reported classification result. The function test
#'   the threshold value of 0.50 0.60 0.65 0.70 0.75 0.80 0.85 0.90 0.95 0.99 if
#'   \code{level} is not provided. Since thresholding is just an option, the
#'   choice of threshold value is really arbitrary and depends on one's
#'   objectives.
...
#' @param saveplot logical. The plot will be saved if \code{TRUE}, and no plot
#'   is displayed in the window. Note that outliers of boxplot are not plotted.
#' @param plotsize numeric. Plot size for the plot saved into file (used only
#'   when \code{saveplot=TRUE}), unit in pixel.
#' @param run argument to be passed to \code{\link{mrkfcv}}
#' @param k argument to be passed to \code{\link{mrkfcv}}
#' @param level numeric vector giving the threshold range to be tested. see details.
...
threcv <- function(X, Y, saveplot = FALSE, plotsize = 1000, run = 30, 
                   k = 5, level) {
  accu <- numeric()
  accu.sd <- numeric()
  total <- numeric()
  ...
  if (missing(level))
    ...
    levellabel <- level
  result <- data.frame(matrix(NA, k * run * length(levellabel), 3))
  threlevel <- gl(length(levellabel), run * k, 
                  labels = levellabel, ordered = TRUE) # group level
  result[, 3] <- threlevel
  for (i in 1:length(levellabel)) {
    cat ("\r                       (Evaluating threshold at: ", levellabel[i], 
         ") | threcv progress: [", round(i / length(levellabel) * 100), 
         "%]       ", sep="")
  ...
  if (saveplot) {
    filename <- paste0(deparse(substitute(X)), "-threshold-optimization(", 
                       Sys.Date(), ").tif")
  ...
  boxplot(result[, 1] ~ result[, 3], outline = FALSE, 
          ...
          at = (1:length(levellabel)) - 0.15, boxwex = 0.2, axes = FALSE,
  ...
  axis(2, at = seq(0, 100, length(levellabel)), las = 2)
  axis(1, at = 1:length(levellabel), labels = levellabel)
  boxplot(result[, 2] ~ result[, 3], border = 4, outline = FALSE, 
          add = TRUE, at = (1:length(levellabel)) + 0.15, boxwex = 0.2, axes=FALSE)
  box()
  if (saveplot) {
    dev.off()
