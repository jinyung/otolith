...
#'  out of all configurations of selected group. Min and max shapes are displayed 
#'  as TPS deformations from the mean shape
#' @param gpa a \code{\link{rGPA}} object
#' @param group factor giving the groupings, must be in same order as \code{A} of 
#'  \code{\link{rGPA}} object
#' @param target character. group that you want to draw, one of the level of \code{group}
#' @param main character. title of the plot. default is using the \code{target}
#' @param n numeric. no. of grids used in TPS visualization, argument passed to \code{tps} 
#' @param col color of the outline
#' @param saveplot logical. Whether to save the plot. No plot will be displayed in window 
#'  if \code{saveplot=TRUE}
#' @param plotsize width of the plot in pixel. Used only when \code{saveplot=TRUE}
