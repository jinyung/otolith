...
//new version to remove the outline extraction, just retain the particle_8 calculation

// Author: Wong Jin Yung

//-------- get largest particle, from KeepLargestParticle.txt(author: G. Landini), updated march 2013--------//
function getlargest () {
...
macro "Batch Outline Extraction" {
	...
	threshold = parts[2]; // third as the threshold value
	...
	for (i=0; i < lengthOf(openList); i++) { 
		...
		// get dimensionless shape indices, using Particle8 plugin by Landini, G.
